        ...
        try {
            ...
            for (ResolvedFieldDeclaration field : typeDecl.getDeclaredFields()) {
                try {
                    ...
                    OutputParam fieldParam = new OutputParam(className, null, name, null);
                    if (ParseRecursiveEndPoint.set.contains(className)) {
                        fieldParam = new OutputParamString(className, name, null, null, new LinkedList<>(), new LinkedList<>());
